...
create_shiny_umap_viewer <- function(umap_file, spectra_file, app_title = "UMAP and MS/MS Spectrum Viewer with History") {
  # 必要なデータをロード
  if (!file.exists(umap_file) || !file.exists(spectra_file)) {
    stop("指定されたファイルが存在しません。")
  }
  
  load(file = umap_file)  # UMAP結果
  load(file = spectra_file)  # スペクトルデータ
  
  if (!exists("umap_result") || !exists("subset_spectra")) {
    stop("入力ファイルに 'umap_result' または 'subset_spectra' が含まれていません。")
  }
...
#   umap_file = "C:/R/umap_pca.rds",
#   spectra_file = "C:/R/subset_spectra_pca.rds",
